...
public class WaypointCircuit : MonoBehaviour {
    ...
    private float[] leftDistances;
    private Vector3[] rightPoints;
    private float[] rightDistances;
    ...
    public float GetRouteDistance(Vector3 pos) {
        int point = 0;
        float minDist = Mathf.Infinity;

        for (int i = 0; i < points.Length; i++) {
            float curDist = Vector3.Distance(pos, points[i]);
            if (curDist < minDist) {
                minDist = curDist;
                point = i;
            }
    ...
    public RoutePoint GetRoutePoint(Vector3 pos) {
        return GetRoutePoint(GetRouteDistance(pos));
    }
